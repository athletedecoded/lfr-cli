...
use aws_sdk_lightsail::types::{AddOnRequest, AddOnType, StopInstanceOnIdleRequest};
...
pub async fn delete_instance(
    lfr_client: LightsailClient,
    instance_name: &str,
) -> DeleteInstanceOutput {
    lfr_client
        .delete_instance()
        .instance_name(instance_name)
        .force_delete_add_ons(true)
        .send()
        .await
        .unwrap()
}
...
pub async fn delete_user_instances(lfr_client: LightsailClient, user: &str) {
    ...
    let user_instances: Vec<String> = all_instances
        .instances
        .clone()
        ...
        .collect();
    let num_instances = user_instances.len();
    println!("Found {} instances for user {}", num_instances, user);
...
pub async fn create_instance(
    ...
    let add_on_request = AddOnRequest::builder()
        ...
        .unwrap();
    // Create instance
    let instance_created = match lfr_client
        .create_instances()
    ...
    if instance_created {
        ...
        if is_running {
            let _ = lfr_client
                ...
                .unwrap();
            let is_stopping =
                probe_state(lfr_client.clone(), &instance_config.name, "stopping").await;
            if is_stopping {
...
pub async fn delete_group(
    ...
    for user in all_users.users {
        ...
        println!(
            "SUCCESS: Deleted user {} from group {}",
...
pub async fn create_user(
    ...
    let user_created = match iam_client
        .create_user()
    ...
    if user_created {
        ...
        println!(
            "SUCCESS: Created user '{}' with unique onetime password: {}",
            &iam_config.user, password
        );
    } else {
